...
class SentenceMatchModelGraph(object):
    def __init__(self, num_classes, lookup_frame, word_vocab=None, char_vocab=None, POS_vocab=None, NER_vocab=None,
                 ...
                 with_full_match=True, with_maxpool_match=True, with_attentive_match=True, with_max_attentive_match=True, embedding_dim = 300):
...
#             self.word_embedding = tf.get_variable("word_embedding", shape=[word_vocab.size()+1, word_vocab.word_dim], initializer=tf.constant(word_vocab.word_vecs), dtype=tf.float32)
            ...
            with tf.device(cur_device):
                ...
                self.word_embedding = tf.get_variable("word_embedding", trainable=word_vec_trainable, shape=[1,1], 
                ...
                self.embedding_placeholder = tf.placeholder(tf.float32, [vocab_size + 2, embedding_dim])
                self.embedding_init = tf.assign(self.word_embedding, self.embedding_placeholder, validate_shape=False)
                        
                #self.word_embedding = tf.get_variable("word_embedding", trainable=word_vec_trainable, shape=[vocab_size,embedding_dim], 
                #                                   initializer=tf.random_uniform_initializer(-1.0, 1.0), dtype=tf.float32)
...
#         gold_matrix = tf.one_hot(self.truth, num_classes)
    ...
    def get_embedding_init(self):
        return self.__embedding_init

    def get_embedding_placeholder(self):
        return self.__embedding_placeholder
    ...
    def set_embedding_init(self, value):
        self.__embedding_init = value

    def set_embedding_placeholder(self, value):
        self.__embedding_placeholder = value
    ...
    def del_embedding_init(self):
        del self.__embedding_init

    def del_embedding_placeholder(self):
        del self.__embedding_placeholder
    ...
    embedding_init = property(get_embedding_init, set_embedding_init, del_embedding_init, "embedding's docstring")
    embedding_placeholder = property(get_embedding_placeholder, set_embedding_placeholder, del_embedding_placeholder, "embedding_placeholder's docstring")
